...
export default function GlobalNav({ onOpenRFQ, onOpenConceptSwitcher }: GlobalNavProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-300">
      {/* Top micro banner for client demo briefing */}
      <div className="bg-black/90 backdrop-blur-md text-stone-300 text-[11px] py-1 px-4 border-b border-white/10 flex items-center justify-between">
        <div className="flex items-center gap-2 max-w-4xl truncate">
          ...
          <span className="hidden sm:inline text-stone-400">
            Compare 4 creative directions for Bangladesh Apparel Buying House.
          </span>
          <span className="font-mono text-amber-300 bg-amber-950/50 px-1.5 py-0.5 rounded border border-amber-800/40 text-[10px]">
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          >
            <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-amber-600 via-rose-700 to-indigo-900 flex items-center justify-center shadow-lg shadow-black/40 ring-1 ring-white/20 group-hover:scale-105 transition-transform duration-300">
              <span className="font-serif font-black text-white text-lg tracking-tighter">TB</span>
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <span className="text-white font-bold tracking-tight text-base group-hover:text-amber-200 transition-colors">
                  THREADWORKS
                </span>
                <span className="text-[10px] tracking-widest uppercase px-1.5 py-0.5 rounded bg-white/10 text-stone-300 font-mono">
                  BD
                </span>
              </div>
              <p className="text-[10px] text-stone-400 tracking-wider uppercase font-medium">
                Dhaka • Gazipur • Chittagong
              </p>
            </div>
          </Link>

          {/* 4 Concept Navigation Pills (Desktop) */}
          <div className="hidden md:flex items-center p-1.5 rounded-full bg-black/60 border border-white/15 backdrop-blur-md shadow-inner">
            {CONCEPTS.map((concept, index) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{index + 1}. {concept.short}</span>
                  {isActive && (
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-amber-500 via-orange-500 to-rose-600 hover:from-amber-400 hover:to-rose-500 text-white text-xs font-bold shadow-lg shadow-orange-950/50 hover:shadow-orange-700/50 hover:scale-105 active:scale-95 transition-all duration-200"
            >
            ...
            <button
              ...
              className="md:hidden p-2 rounded-lg bg-white/5 border border-white/10 text-stone-300 hover:text-white focus:outline-none"
        ...
        {mobileMenuOpen && (
          <div className="md:hidden mt-3 pt-3 border-t border-white/10 space-y-2 bg-stone-950/95 backdrop-blur-2xl rounded-2xl p-4 shadow-2xl border">
