...
export default function FactsFigures() {
  return (
    <section id="key-facts" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#090F18] border-t border-white/10 text-stone-100">
      <div className="max-w-7xl mx-auto space-y-12">
        <div className="text-center max-w-3xl mx-auto space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[#D6CFC7] text-xs font-mono uppercase tracking-widest">
            <TrendingUp className="w-3.5 h-3.5 text-[#E0B27A]" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {FACTS.map((f, i) => {
            ...
            return (
              <div
                key={i}
                className="p-6 sm:p-8 rounded-3xl bg-[#111926] border border-white/10 hover:border-[#E0B27A]/40 transition-all duration-300 space-y-4 shadow-lg hover:shadow-2xl"
              >
                <div className="flex items-center justify-between">
                  <div className="w-10 h-10 rounded-xl bg-black/40 border border-white/10 flex items-center justify-center">
                    <Icon className={`w-5 h-5 ${f.color}`} />
                  </div>
                  <span className="text-[10px] font-mono uppercase px-2.5 py-1 rounded-full bg-white/5 text-[#A59E95] border border-white/5">
                    Metric #{i + 1}
                  </span>
                </div>

                <div>
                  <div className={`text-3xl sm:text-4xl font-serif font-bold ${f.color}`}>
                    {f.figure}
                  </div>
                  <div className="text-xs font-mono text-[#D6CFC7] font-semibold uppercase tracking-wider mt-0.5">
                    {f.unit}
                  </div>
                </div>
                ...
                <div className="pt-2 border-t border-white/10 space-y-1">
                  ...
                  <p className="text-xs text-[#A59E95] leading-relaxed">
                    {f.subtext}
                  </p>
                </div>
              </div>
